...
function FormTarefa({ item, index }: any) {
  ...
  return (
    <Card key={index}>
      <Text style={styles.tituloCard}>{item.maintenance_type_name}</Text>
      <View>
        {item?.file_url && (
          <Image
      ...
      <View style={styles.btnArea}>
        <TouchableOpacity
          onPress={() => {
            if (!item?.file_url) setSelectedRadio(1);
          }}
        >
        ...
        <TouchableOpacity
          onPress={() => {
            if (!item?.file_url) setSelectedRadio(0);
          }}
        >
          <View style={styles.wrapper}>
            <View style={styles.radio}>
      ...
      <Button
        ...
        line={16}
        marginTop={0}
        onPress={renderSaveTarefa}
        load={load}
        active={!item?.file_url}
      >
        <AntDesign
          name={load ? "loading1" : "checkcircleo"}
          size={16}
          color="white"
        />
      </Button>
